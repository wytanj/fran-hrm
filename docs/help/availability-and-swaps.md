---
slug: availability-and-swaps
title: Availability and shift swaps
summary: Submit availability before the cutoff; swap shifts with a teammate subject to approval.
category: scheduling
primary_path: /availability
related_paths: [/swaps, /roster]
intent_tags: [availability, preferences, cannot work, unavailable, shift swap, swap shift, cover my shift, exchange shift, cutoff]
sort_order: 40
---

# Availability and shift swaps

## Submitting availability

1. Open **Availability**.
2. Use the arrows to reach the week you want (it opens on next week).
3. For each day pick **Can work**, **Prefer**, or **Can't**.
4. Adjust the time window if you are only free part of the day.
5. Tap **Submit availability**.

### The cutoff

Availability locks **7 days ahead** by default. Inside that window the roster is already being planned, so the app will refuse the change and tell you to speak to your manager. Supervisors and above can still edit on your behalf.

Part-timers: your availability is what the pool is drawn from. If you submit nothing, you are less likely to be scheduled.

## Shift swaps

1. Open **Shift swaps**.
2. Pick **your shift** to give away.
3. Pick the **teammate** who will take it.
4. Add a reason and request.

### Swap rules

- Requests close **24 hours before** the shift starts.
- You can only offer **your own** shifts.
- A supervisor or store manager must approve. On approval the shift is reassigned automatically — you do not need to do anything else.
- Approved swaps do not change anyone's leave balance or reset OT counters. The hours simply move with the shift.

If the swap is refused, the shift stays yours. Not turning up is a no-show regardless of a pending request.
