import { listAvailability, listAvailabilityLocks } from '../../../../core/roster/query.mjs'
...
export default defineEventHandler(async (event) => {
  ...
  const [data, locks] = await Promise.all([
    listAvailability(db, ctx.workspaceId, args),
    listAvailabilityLocks(db, ctx.workspaceId, args),
  ])
  return { data, locks }
})
