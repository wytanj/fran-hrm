-- 033 â€” Hire gates, payroll_eligible_from, HQ store-cover pay bucket.
-- QR/clock time_entries remain SoT for hours. Roster is plan only.
-- HQ cover convention: time_entries.job_code = 'hq_store_cover' â†’ Additional Wages.
...
create table if not exists public.staff_hire_gates (
  ...
  -- Computed/manual: when all required gates are done; mirrors staff.payroll_eligible_from when set.
...
comment on table public.staff_hire_gates is
  'Hire lifecycle gates (offer â†’ docs/NRIC â†’ start â†’ first QR store presence). Payroll refuses OW until payroll_eligible_from.';
